use mlua::{Value, Table, ThreadStatus};

use super::structures::{scripts::{ScriptsManager, LuaScript}, fs::{LuaFile, LuaFs}};
...
fn pretty_print_Table(val:&Table,depth:Option<i32>) -> String {
    ...
    for pog in val.clone().pairs::<Value,Value>() {
        if let (key, value) = pog.unwrap() {
            ...
            output.push_str(&format!("{}{} : {},\n", "\t".repeat((d+1) as usize), pretty_key, pretty_value));
        }
    }
...
pub fn pretty_print_lvalue(val:&Value,depth:Option<i32>) -> String {
    ...
    match val {
        ...
        Value::Boolean(v) => if v == &true { output = "true".into() } else { output = "false".into() },
        ...
        Value::UserData(ud) => {
            ...
            // ud.
            let meta_name : String = ud.get_metatable().unwrap().get("__name").unwrap_or("unknown".to_string());
            output= format!("userdata<{}> : {}",meta_name, match meta_name.as_str() {
                ...
                "LuaFileSystem" => format!("Allowed directories = {:?}",ud.borrow::<LuaFs>().unwrap().0.join(", ")),
                _ => "{}".to_string()
            })
        },
