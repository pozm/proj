...
fn add_fields<'lua, F: mlua::UserDataFields<'lua, Self>>(fields: &mut F) {
    ...
    fn add_methods<'lua, M: mlua::UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("openDir", |l,t,p:String| {
            ...
            Ok(dir.map(|f|f.unwrap().path().display().to_string()).collect::<Vec<_>>())
        });
        methods.add_method("openFile", |_l,t,p:String| {
            ...
            let file = OpenOptions::new().create(false).write(true).read(true).open(&path)?;
            let file = LuaFile(path.display().to_string(), file);
            Ok(file)
        });
