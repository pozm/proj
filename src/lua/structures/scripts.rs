use std::{collections::HashMap, sync::{Mutex, Arc}, path::Path};

use mlua::{RegistryKey, UserData, Function};
...
impl UserData for LuaScript  {
    fn add_fields<'lua, F: mlua::UserDataFields<'lua, Self>>(fields: &mut F) {
        fields.add_field_method_set("invoke_fn", |lua,this, f : Function<'_>| {
            SCRIPTS_MANAGER.lock().unwrap().fns.insert(this.name.to_string(), Some(
...
impl UserData for ScriptsManager {
    ...
    fn add_fields<'lua, F: mlua::UserDataFields<'lua, Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("scripts", |l,t| {
            ...
            Ok(t.scripts.iter().map(|s|&s.name).cloned().collect::<Vec<String>>())
        });
