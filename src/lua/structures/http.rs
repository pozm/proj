...
use mlua::{UserData, Error, LuaSerdeExt, Value};
use reqwest::{RequestBuilder, Method, header::{HeaderMap, HeaderName}};
...
impl UserData for LuaHttp {
    fn add_methods<'lua, M: mlua::UserDataMethods<'lua, Self>>(methods: &mut M) {
        methods.add_async_method("request", |l,t,options:Value| async move {
            ...
            let result = client.request(match options.method.to_uppercase().as_str() {
                "GET" => Method::GET,
                "POST" => Method::POST,
                "PATCH" => Method::PATCH,
